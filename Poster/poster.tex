...
{\large
\begin{itemize}
\item Various HP/Agilent/Keysight multimeters.
\item DC sources from Yokogawa and Keithley.
\item Lock In Amplifiers from Stanford Research and Signal Recovery.
\item Temperature controllers from Lakeshore and Oxford Instruments.
\item Rohde \& Schwarz RF/Microwave Sources, Spectrum Analyzers, VNAs.
\item and many more...
\end{itemize}
}
\vspace*{1cm}
...
\item live plot at end of measurement
\end{itemize}
...
\vspace*{-2cm}
\begin{itemize}
\item New drivers for R\&S FSV Spectrum analyzer and  R\&S ZVA and ZVM vector
  network analyzers, the HP33120 function generator and the Tektronix TDS2024
  oscilloscope.
\item Automated testing of instrument drivers and high-level functionality via
  mock connection objects. Continuous
  integration testing on both Linux (Travis CI) and Windows (Appveyor).
\item New framework for drivers and connections based on \textbf{Moose}, which
  enables 
  modern object oriented programming (OOP) with Perl5:
  \begin{itemize}
  ...
  \item Organize SCPI subsystems into roles, which can be used by multiple
    drivers. This snippet shows how SCPI roles are consumed by the new R\&S
    FSV driver:
\vspace*{4mm}
\placesmalllisting{fsv-roles.pl}

\vspace*{2cm}
  \item Extensive type system. Convenient validation of user provided
    function parameters.
  \end{itemize}
...
\item \textbf{PDL::Graphics::Gnuplot} gives convenient access to all of
  ...
  and hardcopies (png, jpeg, svg, tikz, pdfcairo, ...).
\end{itemize}
