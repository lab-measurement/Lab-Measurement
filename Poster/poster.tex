...
\begin{itemize}
 ...
 \item
 Lab::Connection --- thin glue layer, one connection per instrument
 \item
 Lab::Instrument --- instrument-level logic, sen\-ding / receiving command strings
 \item 
...
\begin{itemize}
 \item 
 GPIB? Ethernet? doesn't matter here, the connection code takes care of that!
 \item
 fast implementation of many different instrument functions
 \item
 additional features possible, e.g. ``device cache'': stores device parameters
 if setting a value is fast but reading it out is slow / impossible
\end{itemize}
