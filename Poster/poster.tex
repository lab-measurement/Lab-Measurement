...
\vspace{3cm}
\heading{Safe measurement needed?!}
\vspace{-1cm}
\begin{itemize}
\item
Your sample only survives small voltage steps. You make a typo. 
Your sample dies. Noooooooo!
\item
Use `gateprotect' for voltage sources: pre-defined limits for sweep speed, step
size, range
\item
Voltage sweeps, not steps, whenever needed
\end{itemize}

\placelisting{gateprotect.pl}


\vspace{3cm}
\heading{Overview needed?!}
\placefigure{file=images/no-overview}
...
{\tt make\_overview.pl}
\item
Autogenerates a \LaTeX\ file and images, e.g. for all data in a directory
\item
Includes comments, axis labels, ...
\item
``Generate measurement log from data files''
\end{itemize}

\vspace{1cm}
\placefigure{file=images/overview}
...
\vspace*{3cm}
\begin{minipage}{\textwidth}
\verysmallheading{Results obtained using Lab::Measurement}
{\renewcommand{\baselinestretch}{0.3}\footnotesize
\begin{itemize}
...
(2014).
\item S. Ratz, A. K. Hüttel, {\it et al.}, New J. Phys. (accepted) (2014).
