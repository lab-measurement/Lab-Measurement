...
\vspace*{-2cm}
\begin{itemize}
\item New drivers: R\&S FSV spectrum analyzers, R\&S ZVA and ZVM vector
...
\item 
  New framework for drivers and connections based on \textbf{Moose} --- de-facto
  ...
  \item Organize SCPI subsystems into roles, which can be used by multiple
    drivers. Example from the new R\&S FSV spectrum analyzer driver:
\vspace*{4mm}
\placesmalllisting{fsv-roles.pl}
\vspace*{1.5cm}
  \item Extensive type system; easy validation of user provided function 
  parameters.
  \end{itemize}
  

\item 
access to all of gnuplot's \textbf{plot and curve options} via 
