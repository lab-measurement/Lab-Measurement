...
More plot options are documented in L<PDL::Graphics::Gnuplot>.
...
More curve options are documented in L<PDL::Graphics::Gnuplot>.

=head1 Block data


The C<sparam_sweep> method returns 2D PDLs with the following format:
...
We perform a frequency sweep with the VNA for each level of the voltage source:
