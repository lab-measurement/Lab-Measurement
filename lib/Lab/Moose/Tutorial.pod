...
Lab::Measurement::Installation

Lab::Measurement::Backends

Metacpan, Lab::Measurement::Manual

Tutorial Lab::Measurement::Tutorial for the historical interfaces
Lab::Instrument, and Lab::XPRESS.

perldoc

=head1 Quickstart

This tutorial assumes that Lab::Measurement and the required backends are
installed. 

Agilent DMM, RS830 lock-in, Connection types: USB, VXI11, GPIB
...
When using the LAN interface, set the C<connection_type> to I<VXI11> and
...
FIXME: install VISA, see L<Lab::VISA::Installation>.
Lookup instrument in VISA interactive control.
...
When using the LAN interface, set the C<connection_type> to I<VISA::VXI11> and
provide the instrument's IP address with the C<connection_options> hash:
...
We connect the SR830 lock-in amplifier via GPIB. This requires that the NI-488
driver is installed. The GPIB address (primary
address, short B<pad>), is provided in the C<connection_options> hash:
...
Continuous sweeps: config_sweep, trg, wait

=head2 Spectrum and network analyzers

get_spectrum , PDL, unpdl
