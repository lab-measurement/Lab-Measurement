...
sub new {
    ...
    open my $log, ">$path$filename." . $self->configure('output_data_ext')
...
sub import_gpplus {
    ...
    return "What should I import?"
      unless ( ( defined $opts{filename} ) && ( $opts{filename} ne '' ) );

    my ( $filenamebase, $path, $suffix ) =
      fileparse( $opts{filename}, qr/_\d+\.TSK/ );
    ...
    my $meta = new Lab::Data::Meta(
        {
            ...
            dataset_description => 'Imported by Importer.pm on '
              . ( join "-", localtime(time) ),
    ...
    open my $dataout, ">$newpath$newname." . $self->configure('output_data_ext')
      || return "Cannot open output file $newpath$newname."
      . $self->configure('output_data_ext') . ": $!";
    ...
    for my $old_file (@files) {
        ...
        while (<IN>) {
            ...
            if (/^([\d\-+\.Ee]+;)+/) {
                if (/E+37/) {
                    print "Attention: Contains bad data due to overload!\n";
                }
                my @value = split ";";
                $self->log_line( $dataout, @value );
                for ( 0 .. $#value ) {
                    $min[$_] = $value[$_]
                      if ( !( defined $min[$_] ) || ( $value[$_] < $min[$_] ) );
                    $max[$_] = $value[$_]
                      if ( !( defined $max[$_] ) || ( $value[$_] > $max[$_] ) );
                }
                if ( ( $linenum == 0 ) && ( $blocknum == 0 ) ) {
                    ...
                    die
"spaltenzahl scheisse in zeile $linenum von block $blocknum.\n"
                      . "sollte "
                      . 1 + $numcol
                      . " sein. so habe ich keinen bock und sterbe jetzt";
                }
                $linenum++;
                $total_lines++;
            }
                ...
                my ( $time, $date ) = ( $1, $2 );
                $meta->block_description( $blocknum,
                    "Saved at $time on $date" );
    ...
    if ( $opts{archive} ) {
        return "Destination directory {$newpath}imported_gpplus already exists"
          if ( -d $archive_dir );
        ...
        for my $old (@files) {
            ...
            if ( $opts{archive} eq 'move' ) {
                move $old, "$archive_dir/$oldname$oldsuffix"
                  or warn "Cannot move file $old to archive: $!\n";
            }
            else {
                copy $old, "$archive_dir/$oldname$oldsuffix"
                  or warn "Cannot copy file $old to archive: $!\n";
            }
            chmod 0440, "$archive_dir/$oldname$oldsuffix"
              or warn "Cannot change permissions: $!\n";
        }
    }
    return (
        $newpath, $newname, $#files, $total_lines,
        $numcol + 1,
        $blocknum - 1, $archive_dir
    );
}
