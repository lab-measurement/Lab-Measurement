...
my $comment=<<COMMENT;
Strom von 4 nach 13; Ithaco Amp 10^{-7}, Sup 10^{-10}, Avg 0.3ms.
Gates 3 und 5; V_{Gates}=-1.1746V.
...
g0=7.748091733e-5
set title "$title"
set label "Started at $timestamp" at graph 0.02, graph 0.95
...
while ($key ne "s") {
    ...
    print LOG tv_interval ( $start_int, [$s, $ms]),"\t$timestamp\t$read_volt\n";
    $meascount++;
    if ($key eq "m") {
