...
# diese Variante fährt innen Biasspannung
...
my $start         = -0.4;   # 0 => ($gate_1_start, $gate_2_start)
my $end           =  1.7;   # 1 => ($gate_1_end, $gate_2_end)
my $steps         =  300;
...
sub bias_goto {
    my ($inst,$dest)=@_;
    my $volt=$inst->get_voltage();
    for (1..8) {
        my $now=(($dest-$volt)/8)*$_+$volt;
        $inst->set_voltage($now);
        sleep(1);
    }
}
