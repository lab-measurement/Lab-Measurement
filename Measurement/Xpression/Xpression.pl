...
sub importFile {
	...
	foreach my $column (@{$DataSet->{COL_NAMES}}) {
		...
		$frame->{plots}->{$plot_ID}->{column_panels}[$i] = $xrc->LoadPanel($column_container, 'panel_Column');
		
		
		
		$frame->{plots}->{$plot_ID}->{column_panels}[$i]->SetName('panel_Column_'.$i);
		my $label_column_name = &$find('label_ColumnName', $frame->{plots}->{$plot_ID}->{column_panels}[$i]);
		$label_column_name->SetLabel($column);
		
		if ($i % 2) { $frame->{plots}->{$plot_ID}->{column_panels}[$i]->SetBackgroundColour(Wx::Colour->new(240,240,240)); }

		$column_container_sizer->Add($frame->{plots}->{$plot_ID}->{column_panels}[$i], 0, wxEXPAND);
...
sub openPlotDialog {
	...
	if (show_OpenFileDialog($frame, '*.plot', \$path, \$filename) eq wxID_OK) {
...
sub openPlot {
	...
	my $plot_instance = $frame->{Tabs}->GetPage($frame->{Tabs}->GetSelection());
	...
	while ( my ($axis_name,$axis) = each %{$plot} ) 
		...
		if ($axis_name =~ /X|Y|Y2|CB|Z|none/ )
			...
			foreach my $wave (@{$axis->{wave}})
				{
				my $panel = &$find("panel_Column_".($wave->{column_number}-1), $ColumnsContainer);
	...
	$plot_blocks->SetValue("$plot->{BlockFrom}:$plot->{BlockTo}:$plot->{BlockIncrement}");
	
	
	
	
	my $plot_lines = &$find('input_PlotLines', $plot_instance);
	$plot_lines = $plot_lines->SetValue("$plot->{LineFrom}:$plot->{LineTo}:$plot->{LineIncrement}");
...
sub savePlot {
	...
	my $dlg = Wx::FileDialog->new($frame, 'Please select a File...', '', '', '*.plot', 'wxFD_Save, wxFD_OVERWRITE_PROMPT');
	my $result = $dlg->ShowModal();
...
sub AddPlotInstance {
	...
	push (@{$frame->{plot}}, $xrc_plotInstance->LoadPanel($frame->{Tabs}, 'panel_PlotInstance'));
	$frame->{Tabs}->AddPage(@{$frame->{plot}}[-1], "new plot", 1 , 1);

	my $label_ID = &$find( 'label_PlotID' ,@{$frame->{plot}}[-1]);
	$label_ID->SetLabel($frame->{ID_ITERATOR});
	
	my $button_ImportFileDialog = &$find('button_ImportFileDialoge', $frame->{Tabs}->GetPage($frame->{Tabs}->GetSelection()));
	Wx::Event::EVT_BUTTON($frame, $button_ImportFileDialog, \&importFileDialog);
	
	my $button_AddPlotInstance = &$find('button_AddPlotInstance', $frame->{Tabs}->GetPage($frame->{Tabs}->GetSelection()));
	Wx::Event::EVT_BUTTON($frame, $button_AddPlotInstance, \&AddPlotInstance);
	
	my $button_OpenPlot = &$find('button_OpenPlot', $frame->{Tabs}->GetPage($frame->{Tabs}->GetSelection()));
	Wx::Event::EVT_BUTTON($frame, $button_OpenPlot, \&openPlotDialog);
	
	my $button_SavePlot = &$find('button_SavePlot', $frame->{Tabs}->GetPage($frame->{Tabs}->GetSelection()));
	Wx::Event::EVT_BUTTON($frame, $button_SavePlot, \&savePlot);
	
	my $button_ClosePlotInstance = &$find('button_ClosePlotInstance', $frame->{Tabs}->GetPage($frame->{Tabs}->GetSelection()));
	Wx::Event::EVT_BUTTON($frame, $button_ClosePlotInstance, \&ClosePlotInstance);
	
	my $combo_PlotType = &$find('combo_PlotType', $frame->{Tabs}->GetPage($frame->{Tabs}->GetSelection()));
	Wx::Event::EVT_COMBOBOX($frame, $combo_PlotType, \&ChangePlotType);
	
	my $button_Plot = &$find('button_Plot', $frame->{Tabs}->GetPage($frame->{Tabs}->GetSelection()));
	Wx::Event::EVT_BUTTON($frame, $button_Plot, \&Plot);
	
	my $button_ExtractData = &$find('button_ExtractData', $frame->{Tabs}->GetPage($frame->{Tabs}->GetSelection()));
	Wx::Event::EVT_BUTTON($frame, $button_ExtractData, \&export_Data);
	
	my $button_Export = &$find('button_Export', $frame->{Tabs}->GetPage($frame->{Tabs}->GetSelection()));
	Wx::Event::EVT_BUTTON($frame, $button_Export, \&export_Graph);
	
	my $button_PlotBlocks = &$find('spin_PlotBlocks', $frame->{Tabs}->GetPage($frame->{Tabs}->GetSelection()));
	Wx::Event::EVT_SPINCTRL($frame, $button_PlotBlocks, \&update_Plot);
	
	my $button_PlotBlocks = &$find('spin_PlotLines', $frame->{Tabs}->GetPage($frame->{Tabs}->GetSelection()));
	Wx::Event::EVT_SPINCTRL($frame, $button_PlotBlocks, \&update_Plot);
...
sub ClosePlotInstance {
	...
	$frame->{Tabs} = &$find('tabs_main', $frame);
	if ($frame->{Tabs}->GetPageCount() > 1 ) { $frame->{Tabs}->DeletePage( $frame->{Tabs}->GetSelection() ); }
}
...
sub ChangePlotType {
	...
	my $combo_PlotType = &$find('combo_PlotType',  $frame->{Tabs}->GetPage($frame->{Tabs}->GetSelection()));
	my $panel_Y2Axis = &$find('panel_Y2Axis',  $frame->{Tabs}->GetPage($frame->{Tabs}->GetSelection()));
	my $panel_CBAxis = &$find('panel_CBAxis', $frame->{Tabs}->GetPage($frame->{Tabs}->GetSelection()));
	
	my $panel_PlotSetup = &$find('panel_PlotSetup', $frame->{Tabs}->GetPage($frame->{Tabs}->GetSelection()));
	my $sizer_PlotSetup = $panel_PlotSetup->GetSizer();
	
	my $static_PlotBlocks = &$find('static_PlotBlocks', $frame->{Tabs}->GetPage($frame->{Tabs}->GetSelection()));
	my $input_PlotBlocks = &$find('input_PlotBlocks', $frame->{Tabs}->GetPage($frame->{Tabs}->GetSelection()));
	my $spin_PlotBlocks = &$find('spin_PlotBlocks', $frame->{Tabs}->GetPage($frame->{Tabs}->GetSelection()));
	my $static_PlotBlocksExample = &$find('static_PlotBlocksExample', $frame->{Tabs}->GetPage($frame->{Tabs}->GetSelection()));
	
	my $static_PlotLines = &$find('static_PlotLines', $frame->{Tabs}->GetPage($frame->{Tabs}->GetSelection()));
	my $input_PlotLines = &$find('input_PlotLines', $frame->{Tabs}->GetPage($frame->{Tabs}->GetSelection()));
	my $spin_PlotLines = &$find('spin_PlotLines', $frame->{Tabs}->GetPage($frame->{Tabs}->GetSelection()));
	my $static_PlotLinesExample = &$find('static_PlotLinesExample', $frame->{Tabs}->GetPage($frame->{Tabs}->GetSelection()));
...
sub Plot {
	...
	if ( not defined $frame->{plots}->{$plot->{ID}}->{plotter} or not $frame->{plots}->{$plot->{ID}}->{plotter}->available())
		{
		my $plotter = new PlotterGUI($plot);
...
sub update_Plot {
	...
	if ( $plot_blocks->IsShown() )
		...
		$frame->{plots}->{$plot_ID}->{plotdetails}->{CurrentBlockValue} = @{$frame->{plots}->{$plot_ID}->{dataset}->{DATA}[@{$frame->{plots}->{$plot_ID}->{plotdetails}->{X}->{wave}}[0]->{column_number}-1][$Block]}[0];
		if ( abs($frame->{plots}->{$plot_ID}->{plotdetails}->{CurrentBlockValue}) < 1e-3 or abs($frame->{plots}->{$plot_ID}->{plotdetails}->{CurrentBlockValue}) > 1e3 )
			{
			$frame->{plots}->{$plot_ID}->{plotdetails}->{CurrentBlockValue} = sprintf("%1.2e", $frame->{plots}->{$plot_ID}->{plotdetails}->{CurrentBlockValue});
			}
		else
			{
			$frame->{plots}->{$plot_ID}->{plotdetails}->{CurrentBlockValue} = sprintf("%1.3f", $frame->{plots}->{$plot_ID}->{plotdetails}->{CurrentBlockValue});
			}
		$gp .= "set label 1 '".@{$frame->{plots}->{$plot_ID}->{plotdetails}->{X}->{wave}}[0]->{column_name}." = ".$frame->{plots}->{$plot_ID}->{plotdetails}->{CurrentBlockValue}."' at graph 0.5, graph 1.06 front center tc rgb 'white'; ";
		$gp .= "BlockFrom = $Block; ";
		$gp .= "BlockTo = $Block; ";
		$gp .= "BlockIncrement = 1; ";
		}
		...
		$frame->{plots}->{$plot_ID}->{plotdetails}->{CurrentLineValue} = @{$frame->{plots}->{$plot_ID}->{dataset}->{DATA}[@{$frame->{plots}->{$plot_ID}->{plotdetails}->{Y}->{wave}}[0]->{column_number}-1][0]}[$Line];
		if ( abs($frame->{plots}->{$plot_ID}->{plotdetails}->{CurrentLineValue}) < 1e-3 or abs($frame->{plots}->{$plot_ID}->{plotdetails}->{CurrentLineValue}) > 1e3 )
			{
			$frame->{plots}->{$plot_ID}->{plotdetails}->{CurrentLineValue} = sprintf("%1.2e", $frame->{plots}->{$plot_ID}->{plotdetails}->{CurrentLineValue});
			}
		else
			{
			$frame->{plots}->{$plot_ID}->{plotdetails}->{CurrentLineValue} = sprintf("%1.3f", $frame->{plots}->{$plot_ID}->{plotdetails}->{CurrentLineValue});
			}
		$gp .= "set label 1 '".@{$frame->{plots}->{$plot_ID}->{plotdetails}->{Y}->{wave}}[0]->{column_name}." = ".$frame->{plots}->{$plot_ID}->{plotdetails}->{CurrentLineValue}."' at graph 0.5, graph 1.06 front center tc rgb 'white'; ";
		$gp .= "LineFrom = $Line; ";
		$gp .= "LineTo = $Line; ";
		$gp .= "LineIncrement = 1; ";
		}
...
sub get_PlotDetails {
	...
	$plot->{number_of_columns} = $frame->{plots}->{$plot_ID}->{number_of_columns};
	...
	foreach my $item (  @ColumnsContainerChildren )
		...
		@{$plot->{$ColumnAxis}->{wave}}[-1]->{filename} = $frame->{plots}->{$plot_ID}->{path};
	...
	if ( $plot->{type} eq 'Standard' )
		...
		if ( not defined @{$plot->{Y}->{wave}}[0] and not defined @{$plot->{Y2}->{wave}}[0] )
			{
			$plot->{ok} = -1;
			}
		}
		...
		{
		if ( not defined @{$plot->{Y}->{wave}}[0] or not defined @{$plot->{CB}->{wave}}[0] )
			{
...
sub export_Data {
	...
	my $label_ID = &$find( 'label_PlotID' ,$frame->{Tabs}->GetPage($frame->{Tabs}->GetSelection()));	
	my $plot_ID = $label_ID->GetLabel();	
	...
	if ($plot->{ExtractColumns} == 1)
		...
		foreach my $axis ( $plotdetails->{X}, $plotdetails->{Y}, $plotdetails->{Y2}, $plotdetails->{CB}, $plotdetails->{Z} )
			{
			foreach my $wave (@{$axis->{wave}})
				{
	...
	my @blocks = ();
	for  ( my $block = $plotdetails->{BlockFrom}; $block <= $plotdetails->{BlockTo}; $block += $plotdetails->{BlockIncrement} )
		{
		push(@blocks, $block);
		}	
	#$extracted_data = $extracted_data->extract_block(@blocks);
	print "@{$plot->{ExtractBlocks}}\n";
	$extracted_data = $extracted_data->extract_block(@{$plot->{ExtractBlocks}});
	
	
	
	my @lines = ();
	for  ( my $line = $plotdetails->{LineFrom}; $line <= $plotdetails->{LineTo}; $line += $plotdetails->{LineIncrement} )
		{
		push(@lines, $line);
		}
	#$extracted_data = $extracted_data->extract_line(@lines);	
	$extracted_data = $extracted_data->extract_line(@{$plot->{ExtractLines}});	
...
sub show_ExtractDataDialog {
	...
	my $radiobutton_SelectColumns = &$find('radiobox_ExtractColumns', $Dialog);
	Wx::Event::EVT_RADIOBOX($Dialog, $radiobutton_SelectColumns, \&ExtractDataDialog_SelectColumns);
	...
	if ( $Dialog->ShowModal() == wxID_OK )
		...
		my $input_ExtractSelectionLines = &$find("input_ExtractSelectionLines", $Dialog);
		...
		my $input_ExtractSelectionBlocks = &$find("input_ExtractSelectionBlocks", $Dialog);
		...
		my $radioboxExtractColumns = &$find("radiobox_ExtractColumns", $Dialog);
		...
		foreach (0..(my $itemsl = @{$items}))
			...
			if ( $checklist_Columns->IsChecked($_) )
				{
				push(@{$plot->{SelectedColumns}}, @{$plot->{dataset}->{COL_NAMES}}[$_]);
				}
			}
...
sub show_OpenFileDialog {
	...
	my $dlg = Wx::FileDialog->new($frame, 'Please select a File...', '', '', $wildcard, 'wxFD_OPEN, wxFD_FILE_MUST_EXIST');
	my $result = $dlg->ShowModal();
...
sub export_Graph {
	...
	# open File dialog:
	my $dlg = Wx::FileDialog->new($frame, 'Please select a File...', '', '', '(*.png)|*.png|(*.dat)|*.dat|(all)|*.*', 'wxFD_OPEN');
	my $result = $dlg->ShowModal();
