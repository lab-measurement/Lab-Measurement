...
=item * Install cpanm and gnuplot (you can use your package manager).

=item * If you want to use a GPIB Bus you need the Linux-GPIB drivers, library
and Perl bindings. On some distros (e.g. Gentoo) you can use the package
manager to install it and skip the following.

=over

=item * Install the kernel headers. E.g. on Debian you do  
...
=item * Install the perl bindings with

 (as root) cpanm ./language/perl
 
=item * FIXME: add udev rule

=back 

=item * Install Lab::Measurement:

 (as root) cpanm Lab::Measurement
