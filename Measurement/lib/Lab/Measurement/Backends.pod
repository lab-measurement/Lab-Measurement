...
F<INSTALL> file. This guide was tested with Linux-GPIB versions 4.0.2 and
4.0.3 on Debian 8.
...
this command as root:
 ...
 apt-get install linux-headers-`uname -r` build-essential cpanminus 
...
install as root by running:

 make install

=item * Install the Perl bindings as root:
...
F</etc/udev/rules.d/98-gpib-more.rules> with contents:
 ...
 SUBSYSTEMS=="usb", ATTRS{idVendor}=="3923", ATTRS{idProduct}=="709b",\
 ACTION=="add", RUN+="/usr/bin/logger Configured NI USB-GPIB-HS",\
 RUN+="/etc/hotplug/usb/ni_usb_gpib"
