...
sub config_CIRC_sweep {
	...
	if (($x_c, $y_c, $z_c) != ($x[0], $y[0], $z[0]))
		{
		$self->config_DIR_sweep($x[0], $y[0], $z[0], $self->{STARTING_SPEED}, 1, 'C');
	...
	for( my $i=0; $i < $len; $i++)
		...
		($r[$i], $phi[$i], $theta[$i]) = cartesian_to_spherical($X[$i], $Y[$i], $Z[$i]);
		$phi[$i] = ($phi[$i]*180)/pi;
...
sub config_DIR_sweep {
	...
	if ($mode =~ /^(cartesian|CARTESIAN|c|C)$/)
		...
		$mode = 'spherical';
		$B_R = abs($B_R);
		( $x_1, $y_1, $z_1) = spherical_to_cartesian($B_R, pi*$phi/180, pi*$theta/180);
		}
	else {
...
sub create_basic_trace {
	...
	for ( my $i=0; $i < $len; $i++)
		{
		$vx[$i] = abs(($x[($i == $len-1) ? $i : $i+1] - $x[($i == $len-1) ? $i-1 : $i])/(($resolution/$v)));
