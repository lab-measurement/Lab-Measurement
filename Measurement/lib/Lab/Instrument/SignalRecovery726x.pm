...
sub get_value { # basic
	my $self=shift;
	my ($channel, $read_mode, $tail) = $self->_check_args( \@_, ['channel', 'read_mode'] );
	...
	if ( $channel eq "X") {
		...
		$result = $self->query("XY.", $tail).",".$self->query("MP.", $tail);
		$result =~ s/\x00//g;
