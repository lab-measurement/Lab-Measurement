...
our %fields = (
	supported_connections => [ 'VISA', 'VISA_GPIB', 'GPIB', 'RS232', 'VISA_RS232', 'DEBUG' ],
...
sub set_range { # basic
	my $self = shift;
	my ($range, $function) = $self->_check_args( \@_, ['range','function'] );
...
sub set_nplc {# basic
	my $self = shift;
	my ($nplc, $function) = $self->_check_args( \@_, ['nplc','function'] );
...
sub set_averaging { # advanced
	my $self = shift;
	my ($count, $mode, $function) = $self->_check_args( \@_, ['count', 'mode','function'] );
	...
	if ($mode =~ /\b(REPEAT|repeat|MOVING|moving)\b/)
		...
		if ($count >= 0.5 and $count <= 100.5)
			...
			if ($count =~/\b(MIN|min|MAX|max|DEF|def)\b/ or $count =~ /\b\d+(e\d+|E\d+|exp\d+|EXP\d+)?\b/ ) 
				{
				$count = $self->query(":SENSE:$function:AVERAGE:COUNT $count; STATE?; COUNT?; TCONTROL?");
				my $result;
				($result->{'state'}, $result->{'count'}, $result->{'tcontrol'}) = split(/;/, $count);
				return $result;
				}
			}
		elsif ( $count =~ /\b(OFF|off)\b/ or $count == 0 )
			{
			$self->write(":SENSE:$function:AVERAGE:STATE OFF"); # switch OFF Averaging
			$count = $self->query(":SENSE:$function:AVERAGE:STATE?; COUNT?; TCONTROL?");
			my $result;
			($result->{'state'}, $result->{'count'}, $result->{'tcontrol'}) = split(/;/, $count);
			return $result;
			}
...
sub get_averaging {
	...
	my $count = $self->query(":SENSE:$function:AVERAGE:STATE?; COUNT?; TCONTROL?");
	my $result;
	($result->{'state'}, $result->{'count'}, $result->{'tcontrol'}) = split(/;/, $count);
	return $result;
...
sub get_value { # basic
	...
	elsif ($function =~ /\b(PERIOD|period|PER|per|FREQUENCY|frequency|FREQ|freq|TEMPERATURE|temperature|TEMP|temp|DIODE|diode|DIOD|diod|CURRENT|current|CURR|curr|CURRENT:AC|current:ac|CURR:AC|curr:ac|CURRENT:DC|current:dc|CURR:DC|curr:dc|VOLTAGE|voltage|VOLT|volt|VOLTAGE:AC|voltage:ac|VOLT:AC|volt:ac|VOLTAGE:DC|voltage:dc|VOLT:DC|volt:dc|RESISTANCE|resisitance|RES|res|FRESISTANCE|fresistance|FRES|fres)\b/)
		{
...
sub wait { # basic
	...
	while (1)
		{
		if ($self->query(":STATUS:OPERATION:CONDITION?") >= 1024) {last;} # check if measurement has been finished
		else {usleep(1e3);}
		}
...
sub _init_buffer { # internal
	...
	if ( $nop >= 2 && $nop <=1024) {
		$self->write("*CLS");
		$self->write(":STATUS:OPERATION:ENABLE 16"); # enable status bit for measuring/idle status
...
sub _read_buffer { # basic
	...
	#read data
	$self->write(":FORMAT:DATA ASCII; :FORMAT:ELEMENTS READING"); # select Format for reading DATA
