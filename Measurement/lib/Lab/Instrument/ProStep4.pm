...
sub InitEncoder{
	my $self = shift;
	my ($steps, $res, $null) = $self->_check_args( \@_, ['steps', 'res', 'null'] );
...
sub InitRamp{
	...
	my ($axis, $aa, $ae, $va, $ve, $vm) = $self->_check_args( \@_, ['axis', 'aa', 'ae', 'va', 've', 'vm'] );
...
sub move{
	...
	my ($position, $speed, $mode) = $self->_check_args( \@_, ['position', 'speed', 'mode'] );
	...
	if ( $mode eq "ABS" or $mode eq "abs" or $mode eq "ABSOLUTE" or $mode eq "absolute")
		...
		$self->query("ma$AXIS ".$self->angle2steps($position)." $speed\r\n");
		}
		...
		$self->save_motorinitdata();
		$self->query("mr$AXIS ".$self->angle2steps($position)." $speed\r\n");
...
sub wait {
	...
	while ( $self->active())
			...
			if ( $flag <= 1.1 and $flag >= 0.9 )
				...
				{
				print $self->get_id().sprintf(" is          (%.2f\370)\r", $current);
				$flag = 2;
				}
			$flag -= 0.1;
			usleep(2e3);
...
sub init_limits {
	...
	print "----------------------------------------------\n";
	print "\n";
	print "This procedure will help you to initialize the Motor ProStep4 correctly.\n\n";
	print "Steps to go:\n";
	...
	while (1)
		...
		if ($value eq "REF" or $value eq "ref")
			...
			{
			print "Please move the motor position to the REFERENCE POINT. Enter an angle between -188\370 ... +180\370.\n";
			}		
		}
...
sub save_motorinitdata {
	...
	open(DUMP, ">C:\\Perl\\site\\lib\\Lab\\Instrument\\ProStep4.ini"); #open for write, overwrite 
...
sub _save_motorlog {
	my $self = shift;
	my ($init_pos, $end_pos) = $self->_check_args( \@_, ['init_pos', 'end_pos'] );
	open(DUMP, ">>C:\\Perl\\site\\lib\\Lab\\Instrument\\ProStep4.log"); #open for write, overwrite
...
sub read_motorinitdata {
	...
	if (not open(DUMP, "<C:\\Perl\\site\\lib\\Lab\\Instrument\\ProStep4.ini"))
		{
		return 0;
		}
	while(<DUMP>)
