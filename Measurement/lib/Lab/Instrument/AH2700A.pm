...
sub set_bright {
	...
	# read from cache or from device?
	my ($bright1, $bright2, $tail) = $self->_check_args( \@_, ['bright1', 'bright2'] );
...
sub get_single {
	...
	if (wantarray()) {
		return ($values->{C}*1e-12, $values->{L}*1e-9, $values->{V}, $values->{S}, $values->{E});
		}
	else {
...
sub set_field {
	...
	# read from cache or from device?
	my ($fi1, $fi2, $fi3, $fi4, $fi5, $fi6, $tail) = $self->_check_args( \@_, ['fi1', 'fi2', 'fi3', 'fi4', 'fi5', 'fi6'] );
	
	$self->write( sprintf("FIELD %s,%s,%d,%d,%s,%s", $fi1, $fi2, $fi3, $fi4, $fi5, $fi6), $tail);
...
sub set_gpib {
	...
	# read from cache or from device?
	my ($gp1, $gp2, $gp3, $gp4, $gp5, $tail) = $self->_check_args( \@_, ['gp1', 'gp2', 'gp3', 'gp4', 'gp5'] );
	
	$self->write( sprintf("GP %d,%d,%s,%s,%s", $gp1, $gp2, $gp3, $gp4, $gp5), $tail);
...
sub set_date{
	...
	# read from cache or from device?
	my ($yr, $mo, $day, $tail) = $self->_check_args( \@_, ['yr', 'mo', 'day'] );
...
sub set_time{
	...
	# read from cache or from device?
	my ($hr, $min, $sec, $tail) = $self->_check_args( \@_, ['hr', 'min', 'sec'] );
