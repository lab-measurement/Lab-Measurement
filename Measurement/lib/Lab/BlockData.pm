...
subtype 'Lab::BlockData::Natural',
    as 'Int',
    where {$_ >= 0};

# A vector is a non-empty ArrayRef.
subtype 'Lab::BlockData::Vector',
    as 'ArrayRef',
    where {@{$_} > 0};
...
sub row {
    ...
    my ($row) = pos_validated_list(
	\@_,
	{ isa => 'Lab::BlockData::Natural' }
	);
    my $rows = $self->rows();
    if ($row >= $rows) {
...
sub _get_vector_param {
    ...
    my ($vector) = pos_validated_list(
	$args,
	{ isa => 'Lab::BlockData::Vector[Num]' }
	);
