...
sub connection_new {
	...
	if ( exists $args->{'slave_address'} && $args->{'slave_address'} =~ /[0-9]*/ && $args->{'slave_address'} > 0 && $args->{'slave_address'} < 255 ) {
		...
		croak('No or invalid MODBUS Slave Address given. I can\'t work like this!', );
	}
...
sub connection_read { # @_ = ( $connection_handle, $args = { function, mem_address, mem_count }
	...
	if( !defined $function || $function != 3 ) { croak('Undefined or unimplemented function code', ); }
	if( !defined $mem_address || $mem_address < 0 || $mem_address > 0xFFFF ) { croak('Invalid memory address', ); }
	if( $mem_count < 1 ) { croak('Invalid count of registers to be read', ); }
...
sub connection_write { # @_ = ( $connection_handle, $args = { function, mem_address, (int_16)mem_value }
	...
	if( !defined $function || $function != 6 ) { croak('Undefined or unimplemented function code', ); }
	if( !defined $mem_address || $mem_address < 0 || $mem_address > 0xFFFF ) { croak('Invalid memory address', ); }
	if(unpack('n!',$SendValue) != $mem_value) { croak("Invalid Memory Value $mem_value", ); }
