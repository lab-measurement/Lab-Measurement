...
sub save_plot {
	...
	$self->{logger}->{plots}->[$plot_index]->{plotter}->save_plot($type, $filename);
...
sub gnuplot_restart {
	...
	foreach my $plot (@{$self->{logger}->{plots}})
		...
		if (not defined $gpipe)
			{
			$plot->{plotter} = new Lab::XPRESS::Data::XPRESS_plotter($self->{logger}->{filename},$plot);
			$plot->{plotter}->{ID} = $i;
...
sub datazone {
	...
	$self->{logger}->{plots}->[$plot_index]->{plotter}->datazone($x_min, $x_max, $y_min, $y_max, $left, $center, $right);
