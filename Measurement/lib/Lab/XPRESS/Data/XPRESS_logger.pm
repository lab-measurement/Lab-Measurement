...
sub new {
	...
	for( my $i = 0; $i < $num_of_plots; $i++)
		{
		$self->{plots}->[$i]->{plotter} = new Lab::XPRESS::Data::XPRESS_plotter($self->{filename},$self->{plots}->[$i]);
...
sub open_file {
	...
	if ( $filenamebase =~ /(.+)(\/|\/\/|\\|\\\\)(.+)\b/ ) 
		...
		if ( not -d $directory )
			{
			warn "directory given by $filenamebase doesn't exist. Create directory $directory";
			mkdir $directory;
			}
...
sub LOG {
	...
	if ( ref($data) eq 'ARRAY' )
		...
		if ( $number_of_columns >= 1 )
			...
			while ( defined $data[0][$j] )
				...
				for ( my $i=0; $i < $number_of_columns; $i++)
					...
					# else 
						...
						# }
					if ($data[$i][$j] =~ /(^[-+]?[0-9]+)\.?([0-9]+)?([eE][-+]?[0-9]+)?$/)
						{
						print $filehandle sprintf("%+.6e\t",$data[$i][$j]);
						if ( $data[$i][$j] < $self->{DATA}[$i][0] or  not defined $self->{DATA}[$i][0])
							{
							$self->{DATA}[$i][0] = $data[$i][$j];
							}
						elsif ( $data[$i][$j] > $self->{DATA}[$i][1] or  not defined $self->{DATA}[$i][0])
							{
			...
			foreach my $value (@data)
				...
				if ($value =~ /(^[-+]?[0-9]+)\.?([0-9]+)?([eE][-+]?[0-9]+)?$/)
					{
					print $filehandle sprintf("%+.6e\t",$value);
					if ( $value < $self->{DATA}[$i][0] or  not defined $self->{DATA}[$i][0])
							{
							$self->{DATA}[$i][0] = $value;
							}
						elsif ( $value > $self->{DATA}[$i][1] or  not defined $self->{DATA}[$i][0])
							{
							$self->{DATA}[$i][1] =$value;
							}
					#print sprintf("%.6e\t",$value);					
		...
		if ($data =~ /(^[-+]?[0-9]+)\.?([0-9]+)?([eE][-+]?[0-9]+)?$/)
			{
			print $filehandle sprintf("%+.6e\t",$data);
			if ( $data < $self->{DATA}[0][0] or  not defined $self->{DATA}[0][0])
				{
				$self->{DATA}[0][0] = $data;
				}
			elsif ( $data > $self->{DATA}[0][1] or  not defined $self->{DATA}[0][0] )
				{
				$self->{DATA}[0][1] =$data;
				}
			#print sprintf("%.6e\t",$value);
...
#		}	
	...
	for( my $i = 0; $i < $number_of_plots; $i++)
		...
		if (not defined $self->{plots}->[$i]->{plotter}->{plot}->{started} )
			{
			# start plot:
			if ($self->{plots}->[$i]->{plotter}->{plot}->{'type'} eq 'pm3d' and $self->{block_num} <= 1 )
				{
				# start plot later
				}
			else
				{
				$self->{plots}->[$i]->{plotter}->start_plot($self->{block_num});
				}
			}
		else
			{
