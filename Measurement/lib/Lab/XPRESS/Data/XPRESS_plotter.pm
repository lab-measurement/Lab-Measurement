...
sub prepair_plot_config_data {
	...
	if ( exists $self->{COLUMN_NAMES}{$self->{plot}->{'x-axis'}})
		{
		$self->{plot}->{'x-axis'} =  $self->{COLUMN_NAMES}{$self->{plot}->{'x-axis'}};
		}
	...
	if ( exists $self->{COLUMN_NAMES}{$self->{plot}->{'z-axis'}})
		{
		$self->{plot}->{'z-axis'} =  $self->{COLUMN_NAMES}{$self->{plot}->{'z-axis'}};
		}
		
	if ( exists $self->{COLUMN_NAMES}{$self->{plot}->{'cb-axis'}})
		{
		$self->{plot}->{'cb-axis'} =  $self->{COLUMN_NAMES}{$self->{plot}->{'cb-axis'}};
		}
...
sub init_gnuplot {
	...
	my $gpipe = $self->{gpipe};
		...
		if ( not defined $plot{'z-axis'} and not defined $plot{'cb-axis'}  and $plot{'type'} eq 'pm3d')
			{
	...
	foreach my $y (@{$self->{plot}->{'y-axis'}})
		...
		if ( $self->{plot}->{'type'} =~ /\b(line|lines|LINE|LINES|L|l|ln|LN)\b/)
			...
			if ( $block_num > 1 )
				...
				$gp.= "'$filename' using X1:Y1$i every :::0::$old_blocks with lines,";
				$gp.= "'$filename' using X1:Y1$i every :::$current_block"."::$current_block with lines,";
				}
			else
				{
				#my $parameter = $self->{plot}->{'x-axis'};
		...
		elsif ( $self->{plot}->{'type'} =~ /\b(linetrace|LINETRACE|trace|TRACE)\b/)
			{
			#my $parameter = $self->{plot}->{'x-axis'};
			my $block_old = ($block_num-2 < 0) ? $block_num-1 : $block_num-2;
			my $block_new = ($block_num-1 < 0) ? $block_num : $block_num-1;
			$gp.= "'$filename' using X1:Y1$i every :::$block_old"."::$block_old with lines,";
			$gp.= "'$filename' using X1:Y1$i every :::$block_new"."::$block_new with points";
			}
			...
			if ( $block_num > 1 )
				...
				foreach my $block (0..$block_num-1)
					{
					$gp.= "'$filename' using  X1:Y1$i every :::$block"."::$block with lines lt $block ti 'M_$block',";
					}
				}
			else
				{
		...
		elsif ( $self->{plot}->{'type'} =~ /\b(points|point|POINTS|POINT|P|p|pt|PT)\b/ )
			{
			if ( $block_num > 1 )
				{
				#my $parameter = $self->{plot}->{'x-axis'};
				...
				my $current_block = $block_num -1;
				$gp.= "'$filename' using  X1:Y1$i every :::0::$old_blocks with lines,";
				$gp.= "'$filename' using  X1:Y1$i every :::$current_block"."::$current_block with points,";
				}
			else
				{
				#my $parameter = $self->{plot}->{'x-axis'};
			...
			$gp .= "every LineIncrement:BlockIncrement:LineFrom:BlockFrom:LineTo:BlockTo ";
	...
	foreach my $y (@{$self->{plot}->{'y2-axis'}})
		...
		if ( $self->{plot}->{'type'} =~ /\b(line|lines|LINE|LINES|L|l|ln|LN)\b/)
			...
			if ( $block_num > 1 )
				...
				my $old_blocks = $block_num-2;
				my $current_block = $block_num -1;
				$gp.= "'$filename' using X1:Y2$i axis x1y2 every :::0::$old_blocks with lines,";
				$gp.= "'$filename' using X1:Y2$i axis x1y2 every :::$current_block"."::$current_block with lines,";
				}
			else
				{
				#my $parameter = $self->{plot}->{'x-axis'};
			...
			my $block_old = ($block_num-2 < 0) ? $block_num-1 : $block_num-2;
			my $block_new = ($block_num-1 < 0) ? $block_num : $block_num-1;
			$gp.= "'$filename' using X1:Y1$i axis x1y2 every :::$block_old"."::$block_old with lines,";
			$gp.= "'$filename' using X1:Y1$i axis x1y2 every :::$block_new"."::$block_new with points";
			}
		elsif ( $self->{plot}->{'type'} =~ /\b(single|SINGLE)\b/)
			{
			if ( $block_num > 1 )
			...
			if ( $block_num > 1 )
				...
				foreach my $block (0..$block_num-1)
					{
					$gp.= "'$filename' using  X1:Y2$i axis x1y2 every :::$block"."::$block with lines lt $block ti 'M_$block',";
					}
				}
			else
				{
			...
			if ( $block_num > 1 )
				...
				my $current_block = $block_num -1;
				$gp.= "'$filename' using X1:Y2$i axis x1y2 every :::0::$old_blocks with lines,";
				$gp.= "'$filename' using X1:Y2$i axis x1y2 every :::$current_block"."::$current_block with points,";
				}
			else
				{
				#my $parameter = $self->{plot}->{'x-axis'};
...
sub bind_x {
	...
	# draw background:
	$gp .= "set obj 1001 rect from graph 0, graph 0 to graph 1, graph 1 front fc rgb 'grey' fs transparent solid 0.8; ";
	$gp .= "set label 1001 '';\\\n";
	print $gpipe $gp;
	usleep(1e4);
	$gp = "";
	...
	foreach my $column (@columns)
		{
		$gp .= "set obj 100".($m*4+$n+2)." rect from graph (0.05+$n*(0.23)), graph ((1-0.05)-$m*(0.12)) to graph (0.05+$n*(0.23)+0.21), graph ((1-0.05)-$m*(0.12) - 0.1) front fc rgb '#2F3239'; ";
		$gp .= "set label 100".($m*4+$n+2)." '".$column."' at graph (0.05+$n*(0.23) + 0.1), graph ((1-0.05)-$m*(0.12)-0.05) front center tc rgb 'white'; \\\n";
		print $gpipe $gp;
		usleep(2e4);
		$gp = "";
		$n++;
	...
	# check for mouseclicks:
	 ...
	 $gp .= "while(LOOP){";
		...
		$gp = "";
			 foreach my $column (@columns)
				 {
				 $gp .= "if (( MOUSE_X < ( GPVAL_X_MIN + RANGE_X * ".(0.05+$n*0.23+0.21)." )) && ( MOUSE_X > ( GPVAL_X_MIN + RANGE_X * ".(0.05+$n*0.23)." )) && ( MOUSE_Y < ( GPVAL_Y_MIN + RANGE_Y * ".((1-0.05)-$m*0.12)." )) && ( MOUSE_Y > ( GPVAL_Y_MIN + RANGE_Y * ".((1-0.05)-$m*0.12-0.1)." ))) {";
				 $gp .= "LOOP = 0;";
				 $gp .= "X1 = ".($m*4+$n+1)."; ";
...
sub bind_y {
	...
	# draw background:
	$gp .= "set obj 2001 rect from graph 0, graph 0 to graph 1, graph 1 front fc rgb 'grey' fs transparent solid 0.8; ";
	$gp .= "set label 2001 '';\\\n";
	print $gpipe $gp;
	usleep(1e4);
	$gp = "";
	...
	foreach my $column (@columns)
		{
		$gp .= "set obj 200".($m*4+$n+2)." rect from graph (0.05+$n*(0.23)), graph ((1-0.05)-$m*(0.12)) to graph (0.05+$n*(0.23)+0.21), graph ((1-0.05)-$m*(0.12) - 0.1) front fc rgb '#2F3239'; ";
		$gp .= "set label 200".($m*4+$n+2)." '".$column."' at graph (0.05+$n*(0.23) + 0.1), graph ((1-0.05)-$m*(0.12)-0.05) front center tc rgb 'white'; \\\n";
		print $gpipe $gp;
		usleep(2e4);
		$gp = "";
		$n++;
...
sub bind_z {
	...
	# draw background:
	$gp .= "set obj 3001 rect from graph 0, graph 0 to graph 1, graph 1 front fc rgb 'grey' fs transparent solid 0.8; ";
	$gp .= "set label 3001 '';\\\n";
	print $gpipe $gp;
	usleep(1e4);
	$gp = "";
	...
	foreach my $column (@columns)
		{
		$gp .= "set obj 300".($m*4+$n+2)." rect from graph (0.05+$n*(0.23)), graph ((1-0.05)-$m*(0.12)) to graph (0.05+$n*(0.23)+0.21), graph ((1-0.05)-$m*(0.12) - 0.1) front fc rgb '#2F3239'; ";
		$gp .= "set label 300".($m*4+$n+2)." '".$column."' at graph (0.05+$n*(0.23) + 0.1), graph ((1-0.05)-$m*(0.12)-0.05) front center tc rgb 'white'; \\\n";
		print $gpipe $gp;
		usleep(2e4);
		$gp = "";
		$n++;
...
sub bind_c {
	...
	# draw background:
	$gp .= "set obj 4001 rect from graph 0, graph 0 to graph 1, graph 1 front fc rgb 'grey' fs transparent solid 0.8; ";
	$gp .= "set label 4001 '';\\\n";
	print $gpipe $gp;
	usleep(1e4);
	$gp = "";
	...
	foreach my $column (@columns)
		{
		$gp .= "set obj 400".($m*4+$n+2)." rect from graph (0.05+$n*(0.23)), graph ((1-0.05)-$m*(0.12)) to graph (0.05+$n*(0.23)+0.21), graph ((1-0.05)-$m*(0.12) - 0.1) front fc rgb '#2F3239'; ";
		$gp .= "set label 400".($m*4+$n+2)." '".$column."' at graph (0.05+$n*(0.23) + 0.1), graph ((1-0.05)-$m*(0.12)-0.05) front center tc rgb 'white'; \\\n";
		print $gpipe $gp;
		usleep(2e4);
		$gp = "";
		$n++;
...
sub bind_s {
	...
	if ( $self->{FILENAME} =~ /(.+)(\/|\/\/|\\|\\\\)(.+)\b/ ) 
		...
		if ( not -d $directory )
			{
			warn "directory given by  $self->{FILENAME} doesn't exist. Create directory $directory";
			mkdir $directory;
			}
...
sub save_plot {
	...
	if ( $type eq 'eps' )
		...
		{
		warn "in function 'save_plot': file type $type not defined. Possible types are 'png' and 'eps'.";
		}
...
sub datazone {
	...
	$gp .= "set object 1 rect from ".$x_min.",".($y_min + 0.03*($y_max-$y_min))." to ".$x_max.",".($y_min + 0.1*($y_max-$y_min))." behind\n";
					
	$gp .= "set label 1 at ".($x_min + 0.025*($x_max-$x_min)).",".($y_min + 0.06*($y_max-$y_min))." left\n";
	$gp .= "set label 1 '$left'\n";	
		
	$gp .= "set label 3 at ".($x_min + 0.5*($x_max-$x_min)).",".($y_min + 0.06*($y_max-$y_min))." center\n";
	$gp .= "set label 3 '$center'\n";
	
	$gp .= "set label 2 at ".($x_min + 0.975*($x_max-$x_min)).",".($y_min + 0.06*($y_max-$y_min))." right\n";
