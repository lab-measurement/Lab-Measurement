...
sub init_gnuplot {
	...
	my $gpipe = $self->{gpipe};
		...
		if ( $self->{plot}->{X}->{label} eq "")
			{
			$self->{plot}->{X}->{label} = @{$self->{plot}->{X}->{wave}}[0]->{column_name};
		...
		if (  $self->{plot}->{Y}->{label} eq "" )
			...
			if ( defined @{$self->{plot}->{Y}->{wave}}[0] )
				{
				$self->{plot}->{Y}->{label} = @{$self->{plot}->{Y}->{wave}}[0]->{column_name};
				$gp .= "set ylabel '@{$self->{plot}->{Y}->{wave}}[0]->{column_name}'\n";
				}
			}
		else
			{
		...
		if ( $self->{plot}->{Y2}->{label} eq "" )
			...
			if ( defined @{$self->{plot}->{Y2}->{wave}}[0] )
				{
				$self->{plot}->{Y2}->{label} = @{$self->{plot}->{Y2}->{wave}}[0]->{column_name};
				$gp .= "set y2label '@{$self->{plot}->{Y2}->{wave}}[0]->{column_name}'\n";
				}
			}
		else
			{
		...
		if (  $self->{plot}->{CB}->{label} eq "")
			...
			if ( defined @{$self->{plot}->{CB}->{wave}}[0] )
				{
				$self->{plot}->{CB}->{label} = @{$self->{plot}->{CB}->{wave}}[0]->{column_name};
				$gp .= "set cblabel '@{$self->{plot}->{CB}->{wave}}[0]->{column_name}'\n";
				}				
			}
		else
			{
		...
		if (  $self->{plot}->{Z}->{label} eq "")
			...
			if ( defined @{$self->{plot}->{Z}->{wave}}[0] )
				{
				$self->{plot}->{Z}->{label} = @{$self->{plot}->{Z}->{wave}}[0]->{column_name};
				$gp .= "set zlabel '@{$self->{plot}->{Z}->{wave}}[0]->{column_name}'\n";	
				}
			}
		else
			{
		...
		if ( not defined $self->{plot}->{Z}->{wave} and not defined $self->{plot}->{CB}->{wave}  and $self->{plot}->{type} eq 'pm3d')
			{
			warn "Error while plotting data. Plot type = pm3d: z-axis and/or cb-axis are not defined.";
			return;
			}
...
sub plot_standard {
	...
	foreach my $wave (@{$self->{plot}->{Y}->{wave}})
		...
		if ( not defined $wave )
			...
			$gp .= "every $wave->{LineIncrement}:$wave->{BlockIncrement}:$wave->{LineFrom}:$wave->{BlockFrom}:$wave->{LineTo}:$wave->{BlockTo} ";
			$gp .= "axis x1y1 ";
			$gp .= "with $wave->{style} ";
...
sub plot_pm3d {
	...
	if ( defined @{$self->{plot}->{Z}->{wave}}[0])
		{
		$gp = "";
		$gp .= "splot '@{$self->{plot}->{Z}->{wave}}[0]->{filename}' using @{$self->{plot}->{X}->{wave}}[0]->{column_number}:@{$self->{plot}->{Y}->{wave}}[0]->{column_number}:@{$self->{plot}->{Z}->{wave}}[0]->{column_number}; \n";
		}
	elsif (defined @{$self->{plot}->{CB}->{wave}}[0] )
		{
		$gp = "";
		$gp .= "splot '@{$self->{plot}->{CB}->{wave}}[0]->{filename}' ";
		$gp .= "using @{$self->{plot}->{X}->{wave}}[0]->{column_number} : @{$self->{plot}->{Y}->{wave}}[0]->{column_number} : @{$self->{plot}->{CB}->{wave}}[0]->{column_number} ";
		$gp .= "every $self->{plot}->{LineIncrement}:$self->{plot}->{BlockIncrement}:$self->{plot}->{LineFrom}:$self->{plot}->{BlockFrom}:$self->{plot}->{LineTo}:$self->{plot}->{BlockTo} ";
		$gp .= "ti '' ";
		$gp .= ";\n";
		}
...
sub plot_linetraces {
	...
	if ( not defined $mode )
		...
		$gp .= "set obj 1 rect from graph 0, graph 1.11 to graph 1, graph 1.01 front fc rgb '#2F3239'; ";
		$gp .= "set label 1 '' at graph 0.5, graph 1.06 front center tc rgb 'white';; ";
		$gp .= "plot '@{$self->{plot}->{CB}->{wave}}[0]->{filename}' ";
		$gp .= "using @{$self->{plot}->{Y}->{wave}}[0]->{column_number}:@{$self->{plot}->{CB}->{wave}}[0]->{column_number} ";
		...
		$gp .= "set label 1 '' at graph 0.5, graph 1.06 front center tc rgb 'white';; ";
		$gp .= "plot '@{$self->{plot}->{CB}->{wave}}[0]->{filename}' ";
		$gp .= "using @{$self->{plot}->{X}->{wave}}[0]->{column_number}:@{$self->{plot}->{CB}->{wave}}[0]->{column_number} ";
...
sub bind_keys {
	...
	$gp .= "bind 'Right'  'BlockFrom = BlockFrom + BlockIncrement; BlockTo = BlockTo + BlockIncrement; LineFrom = LineFrom + LineIncrement; LineTo = LineTo + LineIncrement; replot;\n' \n";
	$gp .= "bind 'Left' 'if ( BlockFrom > 0 ) { BlockFrom = BlockFrom - BlockIncrement; BlockTo = BlockTo - BlockIncrement; if ( LineFrom > 0 ) { LineFrom = LineFrom - LineIncrement; LineTo = LineTo - LineIncrement; } replot;}' \n";
	#$gp .= "bind 'Up' ' replot;\n' \n";
...
sub names2numbers {
	...
	foreach my $axis ($self->{plot}->{X}, $self->{plot}->{Y}, $self->{plot}->{Y2}, $self->{plot}->{CB}, $self->{plot}->{Z})
		{		
		foreach my $wave (@{$axis->{wave}})
			{						
			if ( exists $self->{plot}->{column_names}{$wave->{column_name}})
				{
				$wave->{column_number} = $self->{plot}->{column_names}{$wave->{column_name}}+1;		
				}
