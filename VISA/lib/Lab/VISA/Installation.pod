...
=head1 Installation on Windows 10 64bit with ActiveState Perl 64bit
...
Download current VISA release from NI (need 64bit version)
...
Check location of visa64.lib (eg. C:\Programme\IVI Foundation\VISA\Win64\lib\msc\visa64.lib)
and remember for later.

=item *

Check directory of visa.h (eg. C:\Programme\IVI Foundation\VISA\Win64\include)
and remember for later.
...
Install Microsoft Visual Studio (large download!). You need C support.

From now on run all commandline programs during installation only from the "Visual Studio Native 64bit Console",
which can be found in the Start menu.

=item

Install ActivePerl 64bit
...
Find and edit C:\Perl64\lib\CORE\config.h: Find the following line 
...
(i.e., remove the last two underscores)

=back

=item

Install gnuplot (not mandatory) from L<http://www.gnuplot.info/>

=over

=item *

Add directory containing C<pgnuplot.exe> to path (if not done automatically):
...
(the location looked up above). I.e., 

    'LIBS' => ['-"lC:\\Program Files\\IVI Foundation\\VISA\\Win64\\lib\\msc\\visa64.lib"']
    'INC'  => '"-IC:\\Program Files\\IVI Foundation\\VISA\\Win64\\include"'    

=item *

Run the following commands in the source directory

    perl Makefile.PL CC=cl
    dmake
    copy visa_wrap.obj visa_wrap.o
    dmake
    dmake install
...
=head1 Installation on Windows XP 32bit with ActiveState Perl 
