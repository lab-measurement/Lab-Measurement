...
the file. For example you could want to plot the sum of two columns. Therefor you could
define a new axis that is defined as the sum of these two columns. An axis value can
be calculated from the values of one or more columns in one line. The expression
C<$C1 + 10 * $C2> defines an axis as the sum of the first column plus ten times
the value of the second column for example. Additionally, axes have labels, units and
such.
