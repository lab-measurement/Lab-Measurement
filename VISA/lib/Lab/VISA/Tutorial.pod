...
is given, but then familarity also with these concepts is assumed.
If you feel the need for more information on Perl or VISA/GPIB, please
see the L</References> section[1-6].
...
error-prone. It's the kind of work that computer were made for.

Another goal is to free the laborant from having to repeat himself all the
time when data is to be presented. If, for example, you are measuring a very
small current with the help of a current amplifier. This current amplifier
...
the same expressions, whenever you make a new plot of your data. So the
C<Lab::Tools> modules also keep track of exactly how to calculate the
current from the measured voltage. This is done by defining an I<axis> that
transforms the values of a I<column>. If you are going to plot I<axes> then,
the transformation will always be done automatically for you, but the
original data will still be left untouched.
...
(3rd column). The the first input value is increased and the next trace is
recorded.

=head3 column

The above example measurements has three columns. You will want to store
additional information for each of these columns: What is beeing set or
...
The example data above was aquired in two I<traces> or I<scans>, which are
...
Usually you will not want the raw data as it is stored in the columns, but
apply some conversations first and the plot the transformed data (as in the
current axample above). This means that you will be plotting I<axes> instead
of I<columns>. An axis value can be calculated from the values of one or
more columns in one line. The expression C<$C1 + 10 * $C2> defines an axis
as the sum of the first column plus ten times the value of the second column
for example. Additionally, axes can have limited ranges that are to be
plotted, labels, units etc. If you really want to plot the raw data from one
of the columns, you just define an axis that has the same values as the
column: C<$C1>.

=head3 plot

=head3 Other fields of interest 
