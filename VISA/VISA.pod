...
It comes even better. The general L<VISA::Instrument> class reduces the
communication overhead to a minimum. And the specialized instrument classes
(virtual instruments) like the L<VISA::Instrument::HP34401A> class offer
...
I have only tested this on Linux so far, but it should work on Windows as well.

Not all API functions are supported, just the ones that I needed.
