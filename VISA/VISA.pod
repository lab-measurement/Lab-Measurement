...
VISA - Perl interface to National Instrument's NI-VISA library
...
Currently only Linux is supported, although support for Windows should be fairly easy.

Currently not all API functions are supported. Only the ones, that I needed.

This manpage only describes the perl syntax of the API. Each function is described
in some sentences that I copied from the NI-VISA Programmer Reference Manual. See this
manual for further description of the library.

=head1 FUNCTIONS

=head2 viClear

    $status=VISA::viClear($vi);

The viClear() operation performs an IEEE 488.1-style clear of the device.

=head2 viClose

    $status=VISA::viClose($object);
...
=head2 viGetAttribute
    ...
    ($status, $attrState)=
        VISA::viGetAttribute($object, $attribute);
...
=head2 viOpen
    ...
    ($status, $vi)=
        VISA::viOpen($sesn, $rsrcName, $accessMode, $openTimeout);
...
=head2 viSetAttribute
    ...
    $status=VISA::viSetAttribute($vi, $attribute, $attrState);
...
Don't know. This swig-based interface is copyright 2004 Daniel Schröer (L<http://www.danielschroeer.de>).
