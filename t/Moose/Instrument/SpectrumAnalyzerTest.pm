...
sub test_spectrum_analyzer {
	...
	skip "set and query the sweep number of points (hardware incable to do so)", 1 if not $cond;
        scpi_set_get_test(
            instr => $s,
    ...
    my $traceXY = $s->get_traceXY(trace => 1); # we must have at least trace one

    is_deeply( [ $traceXY->dims() ], [ $Xpoints_number, 2], "trace has proper dimensions" );
    my $freqs = $traceXY->slice(":, 0");
    is_absolute_error( $freqs->slice("0"), $s->sense_frequency_start_query(), .0001, "frequency start matches trace start");
    is_absolute_error( $freqs->slice("-1"), $s->sense_frequency_stop_query(), .0001, "frequency stop matches trace end");
